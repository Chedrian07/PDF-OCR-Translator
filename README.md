...
변환이 완료된 논문은 **'읽기' 탭**이 기본으로 열립니다 — 왼쪽은 원본 페이지
이미지, 오른쪽은 해당 페이지의 텍스트(번역 완료 후에는 한국어)입니다.

- 페이지 이동: ◀/▶ 버튼, 페이지 번호 입력, 키보드 ←/→
- 확대/축소: A−/A+ (60–220%, 저장됨)
- 번역 전이면 **'한국어로 읽기'** 버튼으로 곧바로 전체 번역을 시작할 수 있고,
  완료되면 자동으로 한국어 뷰로 전환됩니다 ([원문|한국어] 토글로 언제든 대조)
- '질문' 탭은 읽고 있던 페이지를 자동으로 이어받습니다
...
| 형식 | 버튼 | 내용 |
| --- | --- | --- |
| HTML | `HTML` / `레이아웃 HTML` | 단일 파일(이미지 base64·KaTeX 인라인) — 흐름형 문서 뷰 또는 좌표 레이아웃 뷰 |
...
내보내기를 안내합니다. 한글 폰트는 시스템 폰트(macOS AppleSDGothicNeo,
Linux Noto CJK) → PyMuPDF 내장 CJK 순으로 자동 선택되고, `PDF_EXPORT_FONT`로
