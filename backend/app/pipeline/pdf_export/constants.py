...
_MIN_FONT_PT = 4.0
# 최후 수단 축소 사다리. 평상시에는 _SHRINK_STEPS(0.76 하한)와 _MIN_BODY_FONT_PT
# (6.0pt)가 "한 블록만 각주처럼 작아지는 계층 붕괴"를 막는다. 그러나 그 대가가
# **원문 영문을 번역 면에 그대로 남기는 것**이라면 계층 붕괴가 낫다 — 사용자
# 요구는 "번역이 안 되거나 원문과 번역이 같이 있으면 안 된다"이고, 보존은 그
# 요구를 그대로 위반한다. 그래서 다른 모든 시도가 실패한 블록에 한해 이 사다리로
# 한 번 더 시도하고, 성공하면 크기를 경고로 남겨 품질 저하를 관측 가능하게 한다.
_LASTRESORT_SHRINK_STEPS = (0.70, 0.64, 0.58, 0.52, 0.46)
_LASTRESORT_MIN_FONT_PT = 4.5
...
_BLOCK_GAP_PT = 5.0
# 한 번에 거는 리댁션 annot 수. PyMuPDF의 `/NM` 유일성 검사가 삽입마다 페이지의
# 기존 annot을 전수 순회해 페이지당 비용이 2차로 큰다(실측: annot당 0.245ms@100개
# → 2.773ms@1000개). 50개씩 add→apply를 반복하면 순회 대상이 매번 초기화된다
# (실측: 문서 전체 4,190개 기준 6.14s → 1.83s). 100이면 2.36s로 이득이 줄고,
# 더 작게 잡으면 apply_redactions 자체의 고정비가 늘어난다.
_REDACT_CHUNK = 50
