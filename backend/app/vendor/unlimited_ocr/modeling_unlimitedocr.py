...
from transformers.generation import GenerationMixin  # [vendor patch P10]
...
class UnlimitedOCRForCausalLM(DeepseekV2ForCausalLM, GenerationMixin):  # [vendor patch P10]
    ...
    def infer(self, tokenizer, prompt='', image_file='', output_path = '', base_size=1024, image_size=640, crop_mode=True, test_compress=False, save_results=False, eval_mode=False, max_length=32768, tps_interval=0, no_repeat_ngram_size=0, ngram_window=0, temperature=0.0, streamer=None, stopping_criteria=None, logits_processor=None, generate_fn=None):
        ...
        for text_sep, image in zip(text_splits, images):
            ...
            if crop_mode:
                ...
                images_list.append(image_transform(global_view).to(_model_dtype))
                ...
                if width_crop_num > 1 or height_crop_num > 1:
                    ...
                    for i in range(len(images_crop_raw)):
                        images_crop_list.append(image_transform(images_crop_raw[i]).to(_model_dtype))
        ...
        if not eval_mode:
            if streamer is None:  # [vendor patch P3]
            ...
            gen_kwargs = dict(
                input_ids=input_ids.unsqueeze(0).to(_dev),
                images=[(images_crop.to(_dev), images_ori.to(_dev))],
                images_seq_mask=images_seq_mask.unsqueeze(0).to(_dev),
                images_spatial_crop=images_spatial_crop,
            ...
            if logits_processor is not None:  # [vendor patch P3]
                gen_kwargs['logits_processor'] = logits_processor
            elif no_repeat_ngram_size > 0 and ngram_window > 0:
        ...
        if '<image>' in conversation[0]['content'] and save_results:
            ...
            if False and 'line_type' in outputs:  # [vendor patch P8] disabled: eval() of model output is unsafe
    ...
    def infer_multi(self, tokenizer, prompt='', image_files=None, output_path='', image_size=640, save_results=False, max_length=32768, tps_interval=0, no_repeat_ngram_size=0, ngram_window=0, temperature=0.0, streamer=None, stopping_criteria=None, logits_processor=None, generate_fn=None):
        ...
        with _autocast_ctx(_dev, _model_dtype):
            with torch.no_grad():
                gen_kwargs = dict(
                    input_ids=input_ids.unsqueeze(0).to(_dev),
                    images=[(dummy_crop.to(_dev), images_ori.to(_dev))],
                    images_seq_mask=images_seq_mask.unsqueeze(0).to(_dev),
                    images_spatial_crop=images_spatial_crop,
                    do_sample=temperature > 0,
                    temperature=temperature if temperature > 0 else None,
