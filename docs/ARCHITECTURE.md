...
```
├── docker-compose.yml          # ocr-cpu(기본), ocr-cuda(cuda 프로필)
...
│   │   │   ├── registry.py     # 디바이스·엔진 선택 (unlimited/fake/ovisocr2/paddleocr_vl)
...
```json
{
  ...
  "qa_available": true,             // 페이지 Q&A 사용 가능 여부 (LLM 공급자 레이어 — §17)
...
| `OPENAI_API_KEY` | (없음) | 번역 프로바이더 API 키 (로컬 서버는 생략 가능) |
...
- **단일 GPU 원칙**: cuda/ovis/paddle 스택은 동시 기동 금지 (16GB VRAM 경쟁).
  sidecar 캐시 볼륨은 스택별 분리(`ovis-hf-cache`, `paddle-hf-cache`, `paddle-x-cache`)
- 두 서비스 모두 포트를 **루프백(127.0.0.1)에만 바인딩** — LAN 공개는 opt-in (§14 보안)
...
- **POST /api/jobs/{id}/translate** — body `{"lang":"ko","force":false}` (기본 `lang="ko"`).
  - `400` 지원하지 않는 언어 / `409` 변환이 완료된 잡만 번역 가능 / `503` 프로바이더 미설정(detail=사유)
...
- **인증 없음**: 모든 REST/SSE 엔드포인트가 무인증 — 접근 가능하면 전 문서 열람
  (`GET /api/jobs`)·삭제·변환·(설정 시) 유료 번역 트리거가 가능하다. 그래서 compose는
  포트를 **127.0.0.1에만 바인딩**한다(§8) — 신뢰 경계는 "로컬 머신"이다.
- **Host 헤더 화이트리스트**: `TrustedHostMiddleware`가 `ALLOWED_HOSTS`
  (기본 `localhost,127.0.0.1`) 밖의 Host를 400으로 거부 — 악성 웹페이지가
  DNS rebinding으로 same-origin을 획득해 로컬 인스턴스의 문서를 읽어가는 것을 차단한다.
  Starlette는 포트를 떼고 비교하므로 `localhost:8000`도 통과하고, 컨테이너 내부
  healthcheck(`curl http://localhost:8000/api/health`)도 기본값으로 동작한다.
  테스트는 conftest가 테스트 프로세스의 기본 화이트리스트에만 `testserver`를 추가한다.
- **LAN/공개 배포는 opt-in**: 인증을 제공하는 리버스 프록시(nginx basic auth 등)나
  VPN 뒤에 두고, compose ports를 `"8000:8000"`으로 바꾸고 접속 호스트명/IP를
  `ALLOWED_HOSTS`에 추가한다(compose `environment`에 항목 추가 필요). README §보안 참조.
...
- **GET /api/providers** — 공급자 카탈로그. 공급자별
  ...
  OpenAI 계열은 `OPENAI_API_KEY` 유무로, Ollama는 로컬 데몬의 모델 목록
...
- **POST /api/jobs/{id}/qa** — **현재 페이지에서 추출된 텍스트만** 문맥으로 질의.
  ...
  공급자 미구성 시 503 (번역 §13과 같은 패턴 — 나머지 기능은 그대로 동작).
- **GET /api/health** 추가 필드(추가만 — 기존 필드 의미 불변): `qa_available`,
  `llm_default_provider` (§5 참조).
...
- **`OPENAI_API_KEY`는 공유**: 번역과 Q&A(OpenAI 공급자)가 같은 키를 읽는다 —
  Q&A 전용 키는 없다.
